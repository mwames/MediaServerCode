...
public:
    ...
    **/
   MediaClient(std::string name, mediastub *p_cc) : MediaClientGui(name.c_str()) //constructor
   {
      cc = p_cc;
   ...
   void TreeCallback() 
   {
      ...
      {  // reason callback was invoked
        ...
        {
	   temp = cc->get(item->label());
	   isolate = temp[0];
           aStr = "selection";
           titleInput->value(aTitle.c_str());
           albumInput->value(isolate.get("album", "").asString().c_str());
	   genreInput->value(isolate.get("genre", "").asString().c_str());
	   authorInput->value(isolate.get("author", "").asString().c_str());
	   if(isolate.get("mediaType","").compare("0") == 0)
              mediaType->value(0);
	   else
	      mediaType->value(1);
           break;
...
///////////// Handle menu selections
	...
	else if(selectPath.compare("Media/Add")==0) // make a call to existing add function
	{
        cout << "Menu item Add selected." << endl;
        cout << "Adding media with title: " << titleInput->value() << " type " << mediaType->value() << " " << endl;
	
	isolate.clear();
	isolate["album"] = albumInput->value();
	isolate["author"] = authorInput->value();				
	isolate["genre"] = genreInput->value();		
	isolate["mediaType"] = mediaType->value();				
	isolate["title"] = titleInput->value();
	bool addRes = cc->add(isolate);
	isolate.clear();
        cout << "Add " << ((addRes)?"successful":"unsuccessful") << endl;
	buildTree();
        }

	else if(selectPath.compare("Media/Remove")==0) // make a call to existing remove function
	{
        cout << "Menu item Remove selected." << endl;
        cout << "Removing media with title: " << titleInput->value() << " type " << mediaType->value() << " " << endl;
        bool addRes = cc->remove(titleInput->value());
        cout << "Add " << ((addRes)?"successful":"unsuccessful") << endl;
	buildTree();
	tree->redraw();
        }
	...
	else if(selectPath.compare("Media/Play")==0)
	{
         ...
         std::cout << "OS type is: " << unameres << " curr.dir is: " << pwdPath << std::endl;
         // This path is only valid on linux so we will have to check ostype
         std::stringstream streamLinux;
         streamLinux << "/usr/bin/vlc " << pwdPath << "/" << ((mediaType->value()==0)?"PaperNavySwanSong.mp3": "MachuPicchuTimelapseVimeo.mp4");
         std::string aStr("Linux");
         std::stringstream streamMac;
         streamMac << "/Applications/VLC.app/Contents/MacOS/VLC "<< pwdPath << "/" << ((mediaType->value()==0)?"PaperNavySwanSong.mp3": "MachuPicchuTimelapseVimeo.mp4");
   ...
   void populateLists()
   {
      ...
      {
         titles.push_back(temp[i].asString());
      }
      temp.clear();

      //populate list of music titles
      temp  = cc->getMusicTitles();
      for(int i = 0; i < temp.size(); i++)
      {
         musicTitles.push_back(temp[i].asString());
      }
      temp.clear();

      //populate list of videoTitles
      temp = cc->getVideoTitles();
      for(int i = 0; i < temp.size(); i++)
      {
         videoTitles.push_back(temp[i].asString());
      }
      temp.clear();
   }
...
int main(int argc, char * argv[]) 
{
   ...
   cout << "host address is " << host << endl;
   HttpClient httpclient(host);
   mediastub cc(httpclient);
   mediastub *p_cc = &cc;
   cout << "Connecting to host " << host << endl;
   std::string nameStr = (argc>1)?argv[1]:"Media Library";
   MediaClient mc(nameStr, p_cc);
   //mc.setPointer();
   return (Fl::run());
}
